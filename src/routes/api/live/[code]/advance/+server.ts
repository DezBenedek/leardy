...
import { advanceOnce, getLiveByCode, transition } from '$lib/server/live';

// POST /api/live/[code]/advance — tanári "Tovább" (csak globális léptetésnél).
export const POST: RequestHandler = async (event) => {
	...
	const next = await advanceOnce(db, cur);
	return json({ ok: true, status: next.status, current_idx: next.current_idx });
};
